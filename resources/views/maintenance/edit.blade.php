...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h3 class="fw-bold m-0"><i class="bi bi-pencil-square"></i> Edit Maintenance</h3>
            ...
            <div class="row g-3">
                    ...
                    <select name="infrastruktur_id" class="form-select @error('infrastruktur_id') is-invalid @enderror" required>
                        ...
                        @foreach($infrastruktur as $infra)
                            <option value="{{ $infra->id }}"
                                {{ old('infrastruktur_id', $data->infrastruktur_id) == $infra->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <select name="petugas_id" class="form-select @error('petugas_id') is-invalid @enderror" required>
                        ...
                        @foreach($petugas as $p)
                            <option value="{{ $p->id }}"
                                {{ old('petugas_id', $data->petugas_id) == $p->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="tanggal_maintenance"
                        ...
                        value="{{ old('tanggal_maintenance', \Carbon\Carbon::parse($data->tanggal_maintenance)->format('Y-m-d')) }}"
                        required>
                ...
                <div class="col-md-6">
                    ...
                    <select name="kondisi_setelah" class="form-select @error('kondisi_setelah') is-invalid @enderror" required>
                        ...
                        <option value="baik" {{ old('kondisi_setelah', $data->kondisi_setelah) == 'baik' ? 'selected' : '' }}>Baik</option>
                        <option value="rusak_ringan" {{ old('kondisi_setelah', $data->kondisi_setelah) == 'rusak_ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                        <option value="rusak_berat" {{ old('kondisi_setelah', $data->kondisi_setelah) == 'rusak_berat' ? 'selected' : '' }}>Rusak Berat</option>
                    </select>
                ...
                <div class="col-12">
                    ...
                    <textarea name="hasil_pemeriksaan" rows="4"
                        ...
                        placeholder="Deskripsikan hasil pemeriksaan dan pekerjaan yang dilakukan..."
                        required>{{ old('hasil_pemeriksaan', $data->hasil_pemeriksaan) }}</textarea>
                ...
                <div class="col-12">
                    ...
                    <textarea name="catatan" rows="3"
                        ...
                        placeholder="Catatan tambahan (opsional)...">{{ old('catatan', $data->catatan) }}</textarea>
                    @error('catatan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
